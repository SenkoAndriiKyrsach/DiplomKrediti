...
import "../manager/ManagerProductListPage.css";
        ...
        <tbody>
          {products.map((p) => (
            <tr key={p.product_id} className={!p.is_active ? "row-inactive" : ""}>
              ...
              <td>{p.min_amount.toLocaleString("uk-UA")} – {p.max_amount.toLocaleString("uk-UA")}</td>
              ...
              <td className="prod-actions">
                <button
                  className="edit-btn"
                  onClick={() => navigate(`/admin/product/${p.product_id}`)}
                >
                  Редагувати
                </button>
                {p.is_active && (
